...
import { isDeskAlreadyBookedByUser, isResourceAvailable, getResourceBookingsForDate, timeToMinutes, minutesToTime } from '../utils';
...
export const BookingModal: React.FC<BookingModalProps> = ({ resource, selectedDate, onClose, existingBooking }) => {
  const { currentUser, bookings, addBooking, updateBooking, resources, isAdmin, updateResource, deleteResource, deleteBooking } = useAppContext();
  
  // Get the most up-to-date resource from context
  ...
  const handleSave = () => {
    ...
    // Validation 1: Resource availability
    ...
    // Validation 2: Double desk booking
    if (resource.type === 'desk') {
      ...
      if (isDeskAlreadyBookedByUser(otherBookings, currentUser.id, date, startTime, endTime, resources)) {
        setError('You already have a desk booked for this time.');
        return;
      }
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            ...
            <p className="text-sm text-gray-500 capitalize">{currentResource.type}</p>
          </div>
          ...
          <div className="space-y-5">
            ...
            {resource.type === 'desk' ? (
              ...
              <div>
                ...
                {/* Daily Schedule Timeline for Rooms */}
                <div className="mt-6">
                  <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Today's Schedule</h4>
                  {dayBookings.length === 0 ? (
                    <p className="text-sm text-gray-500 italic">No other bookings for this day.</p>
                  ) : (
                    <div className="space-y-2">
                      {dayBookings.sort((a, b) => timeToMinutes(a.startTime) - timeToMinutes(b.startTime)).map(b => (
                        <div key={b.id} className="flex items-center gap-3 text-sm p-2 bg-gray-50 rounded border border-gray-100">
                          ...
                          <div className="text-gray-700 truncate">Booked</div>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            {existingBooking && (
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                  isConfirmingBookingDelete
                    ? 'text-white bg-red-600 hover:bg-red-700'
                    : 'text-red-600 bg-white border border-red-200 hover:bg-red-50'
                }`}
              >
                {isConfirmingBookingDelete ? 'Confirm Delete' : 'Delete Booking'}
              </button>
            )}
            <button
              onClick={onClose}
